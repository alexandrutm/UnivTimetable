...
ClassTableModel::ClassTableModel(Context& aContext, QObject* parent): QAbstractTableModel(parent), mContext(aContext) { }


int ClassTableModel::rowCount(const QModelIndex& parent) const { return mContext.GetClassSize(); }

int ClassTableModel::columnCount(const QModelIndex& parent) const { return 2; }
...
void ClassTableModel::PopulateModel(QString aName, int aNrOfStudents)
{
  ...
  shared_ptr<Classes> newClass = make_shared<Classes>(aName.toStdString(), aNrOfStudents, mContext.GenerateClassId());
