...
TableModel::TableModel(Context& aContext, QObject* parent): QAbstractTableModel(parent), mContext(aContext) { }


int TableModel::rowCount(const QModelIndex& parent) const { return parent.isValid() ? 0 : mContext.GetTeacherSize(); }

int TableModel::columnCount(const QModelIndex& parent) const { return parent.isValid() ? 0 : 2; }
...
void TableModel::PopulateModel(QString aFirstName, QString aLastName)
{
  ...
  shared_ptr<Teacher> newTeacher
    = make_shared<Teacher>(aFirstName.toStdString(), aLastName.toStdString(), mContext.GenerateTeacherId());
