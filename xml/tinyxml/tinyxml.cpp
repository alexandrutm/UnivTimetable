...
int TiXmlElement::QueryUnsignedAttribute(const char * name, unsigned * value1) const
{
  const TiXmlAttribute * node = attributeSet.Find(name);
  if (!node)
    return TIXML_NO_ATTRIBUTE;
...
bool TiXmlDocument::LoadFile(FILE * file, TiXmlEncoding encoding)
{
  ...
  */
  ...
  // in decimal) or CR (Carriage return, '\r', 0x0D, 13 in decimal) individually, or CR followed by
  // LF (CR+LF, 0x0D 0x0A)...
